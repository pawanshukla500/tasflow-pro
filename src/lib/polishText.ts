 ...
 * Gemma/instruction models sometimes dump reasoning; strip that to the final sentence.
 */
...
/** Try parse {"corrected":"..."} / {"polished":"..."} / {"text":"..."} */
...
/**
 * Given original note + model raw output, return only the corrected note text.
 */
export function extractPolishedText(raw: string, original?: string): string {
  ...
  if (looksLikeAnalysis(text)) {
    // Prefer last quoted phrase that isn't a label
    ...
    // Last non-empty line that doesn't look like a label / arrow analysis
    const lines = text
      .split(/\n/)
      .map((l) => l.trim())
      .filter(Boolean)
