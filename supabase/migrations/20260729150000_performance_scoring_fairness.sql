-- Performance scoring fairness: remove creator double-counting, fix weight math,
-- and stop docking people for factors that do not apply (e.g. no workflows).
--
-- Bugs fixed:
--   1. Creators were scored on tasks they assigned to others (credit + overdue blame).
--      Score now uses assigned work only; unassigned self-created tasks still count.
--   2. Component weights summed to 105% (30+25+15+15+10+5+5).
--   3. Missing workflows / zero completions used rate=0 while still weighted,
--      permanently docking task-only users ~15 pts and new assignees on on-time.
--   4. Quality defaulted to 100 with no reviews (free points); now excluded + renormalized.
--
-- performance_events.points_delta remains an audit trail only — the live score
-- already reflects late work via on_time_rate / overdue_penalty_score.
...
DECLARE
  ...
  -- Base weights sum to 100; inactive factors are dropped and remaining renormalized.
  w_task NUMERIC := 0;
  w_ontime NUMERIC := 0;
  w_overdue NUMERIC := 0;
  w_wf NUMERIC := 0;
...
BEGIN
  ...
  -- Tasks you are assigned to, OR unassigned tasks you created (you still own the work).
  -- Delegated work (created_by you, assigned to someone else) does NOT count for you.
  WITH user_tasks AS (
  ...
  ELSE
    ...
    -- Activate only applicable factors (base weights sum to 100).
...
-- Keep completion trigger aligned: always recalc assignees; creator only when
-- unassigned (creator still owns the work) or when creator is also an assignee
-- (covered by the assignee loop).
...
BEGIN
  IF NEW.status = 'done' AND (OLD.status IS DISTINCT FROM 'done') THEN
    ...
    -- Unassigned task: creator owns the work for scoring purposes.
...
-- Recalc assignees on task changes; only recalc creator when they still own
-- scoring for the task (unassigned) or are themselves an assignee.
...
BEGIN
  ...
  IF NOT v_has_assignees AND NEW.created_by IS NOT NULL THEN
    PERFORM public.recalculate_user_performance(NEW.created_by);
  ELSIF NEW.created_by IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM public.task_assignees WHERE task_id = NEW.id AND user_id = NEW.created_by
  ) THEN
    -- Creator delegated this task — refresh their score so delegated work drops out.
    PERFORM public.recalculate_user_performance(NEW.created_by);
  END IF;
...
-- Backfill all active users with the corrected formula
DO $$
DECLARE r RECORD;
BEGIN
